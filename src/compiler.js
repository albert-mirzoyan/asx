...
class Compiler {
        ...
        files.forEach(f=> {
            ...
            switch (file.type) {
                ...
                case 'resource' :
                    ...
                    break;
                case 'ignore'   :
                    console.info('ignored ' + file.path);
                    break;
            }
        });
    ...
    writeSource(out,file){
        if (file.output) {
            Files.writeFile(Files.resolve(out, file.path), file.output);
            Files.writeFile(Files.resolve(out, file.name+'/out.js'), file.output);
        }
        if(file.source){
            Files.writeFile(Files.resolve(out, file.name+'/src.js'), file.source);
        }
        if (file.ast) {
            if (file.ast.src) {
                Files.writeFile(Files.resolve(out, file.name+ '/src.ast'), file.ast.src);
            }
            if (file.ast.out) {
                Files.writeFile(Files.resolve(out, file.name+ '/out.ast'), file.ast.out);
            }
        }
    }
}
